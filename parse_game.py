...
def bb2array_2(b,flip=False):
    x = numpy.zeros(64,dtype=numpy.int8)
    # we need to iterate over the pieces at a row and column
    for piece in range(7):
        for color in range(1):
            if piece != 0:
                for p in b.pieces(piece,color):
                    color = int(bool(b.occupied_co[chess.BLACK] & chess.BB_SQUARES[p]))
                    col = int(p % 8)
                    row = int(p / 8)
                    if flip:
                        row = 7-row
...
def parse_game(g):
    ...
    x = bb2array_2(b, flip=flip)
    b_parent = gn.parent.board()
    x_parent = bb2array_2(b_parent, flip=(not flip))
    if flip:
        y = -y
    ...
    b_parent.push(move)
    x_random = bb2array_2(b_parent, flip=flip)
